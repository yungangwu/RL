...
class Agent(ABC):
    def __init__(self, agent_id: int, policy: BasePolicy, agent_config: dict) -> None:
        ...
        self.policy = policy
        self.batch: Batch = Batch(obs={}, act={}, rew=0, done=False,
    ...
    def step(self) -> None:
        assert self.get_status() == AgentStatus.REWARDED
        if self.policy.training:
            self.batch.update(info={})
            ptr, ep_rew, ep_len, ep_idx = self._buffer.add(self.batch)
        else:
            ptr, ep_rew, ep_len, ep_idx = 0, 0, 0, 0
        self.batch.obs = self.batch.obs_next
        self.batch.info = self.get_info()
        self._set_status(AgentStatus.WAIT_TO_ACT)
