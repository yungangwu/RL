...
class SelfPlayTest:
    ...
    def __init__(self, Game, config, configs, seed, agents):
        ...
        # self.observation_shapes = self.config.observation_shapes
        # self.agents = []
        # for i in range(self.config.num_players):
        #     self.model = models.MuZeroNetwork(self.configs[i])
        #     self.model.set_weights(initial_checkpoint[i].checkpoint["weights"])
        #     self.model.to(torch.device("cuda" if self.config.selfplay_on_gpu else "cpu"))
        #     self.model.eval()
        #     self.agents.append(self.model)
        
        # # Set models for multiplayers game.
        # self.game.set_agents(self.agents)
        # self.observation = self.game.reset()

    def continuous_self_play(self, shared_storage, replay_buffer, test_mode=False):
        player_id = self.game.to_play()
        while ray.get(
            shared_storage.get_info.remote(player_id,"training_step")
        ) < self.config.training_steps and not ray.get(
            shared_storage.get_info.remote(player_id,"terminate")
        ):
            # Set the latest weights for all models
            for i in range(self.config.num_players):
                self.agents[i].set_weights(ray.get(shared_storage.get_info.remote(i,"weights")))

            if not test_mode:
                game_historys = self.play_game(
                ...
                replay_buffer.save_game.remote(game_historys, shared_storage)
                ...
                for i in range(self.config.num_players):
                    shared_storage.set_info.remote(
                        i,
                        {
                            "episode_length": len(game_historys[i].action_history) - 1,
                            "total_reward": sum(game_historys[i].reward_history),
                            "mean_value": numpy.mean(
                                [value for value in game_historys[i].root_values if value]
                            ),
                        }
                    )
                    ...
                    shared_storage.set_info.remote(
                        i,
                        {
                            "muzero_reward": sum(
                                ...
                                # if game_historys[i].to_play_history[k - 1]
                                # == self.config.muzero_player
                            ),
                            "opponent_reward": sum(
                                reward
            ...
            if not test_mode and self.config.ratio:
                while (
                    ray.get(shared_storage.get_info.remote(player_id,"training_step"))
                    / max(
                        1, ray.get(shared_storage.get_info.remote(player_id,"num_played_steps"))
                    )
                    < self.config.ratio
                    and ray.get(shared_storage.get_info.remote(player_id,"training_step"))
                    < self.config.training_steps
                    and not ray.get(shared_storage.get_info.remote(player_id,"terminate"))
                ):
                    time.sleep(0.5)

            if player_id < self.config.num_players - 1:
                player_id += 1
            else:
                player_id = 0
    ...
    def test(self, num_test, temperature, temperature_threshold, render, opponent, muzero_player):
        results = []
        for i in range(num_test):
            game_historys = self.play_game(temperature, temperature_threshold, render, opponent, muzero_player)
            results.append(game_historys)
