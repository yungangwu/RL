...
from policy.base_policy import DDZPolicy
...
class DDZPPO(DDZPolicy):
    def __init__(self, policy_config, model, old_model, optim) -> None:
        super(DDZPPO, self).__init__()
        self.config = policy_config
        self.model = model
        self.old_model = old_model
        self.optim = optim
        self._device = policy_config.get('device', 'cpu')
...
def init_policy(**kwargs):
    ...
    ppo_policy = DDZPPO(config=policy_config, model=model, old_model=old_model, optim=optim)
    if checkpoint:
