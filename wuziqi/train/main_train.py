...
from model.model_ppo import PPOPolicyValue
from player.mcts_player import MCTSPlayer, MCTS_Pure
from player.player import StrategyPlayer
...
class TrainPipeline:
    def __init__(self, ) -> None:
        ...
        self.policy_value_net = PPOPolicyValue(model_file='', a_dim=board_width*board_height)
        self.player = StrategyPlayer(strategy=self.policy_value_net, is_selfplay=1, player_name='PPO')
    ...
    def collect_selfplay_data(self, n_games=1):
        for i in range(n_games):
            winner, play_data = self.env.start_self_play(self.player)
    ...
    def policy_update(self):
        meta_data = self.replay_buffer.sample(train_batch_size)
    ...
    def policy_evaluate(self, n_games=10, old_model_path='', new_model_path=''):
        ...
        if os.path.exists(old_model_path) and os.path.exists(new_model_path):
            old_strategy = PPOPolicyValue(model_file=old_model_path, a_dim=board_width*board_height)
            old_dqn_player = StrategyPlayer(strategy=old_strategy, is_selfplay=1, player_name='old_ppo')

            new_strategy = PPOPolicyValue(model_file=new_model_path, a_dim=board_width*board_height)
            current_dqn_player = StrategyPlayer(strategy=new_strategy, is_selfplay=1, player_name='new_ppo')
            ...
            old_dqn_player = StrategyPlayer(strategy=self.policy_value_net, is_selfplay=1, player_name='old_ppo')

            current_dqn_player = StrategyPlayer(strategy=self.policy_value_net, is_selfplay=1, player_name='new_ppo')
