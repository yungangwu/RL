    ...
    def play_one_game(self, log=False):
        ...
        while True:
            ...
            if winner is not None:
                if log:
                    print('Game over, Winner is ', 'black' if winner == Player.BLACK.value else 'white')
                return winner
...
if __name__ == '__main__':
    ...
    agent1 = DQNAgent(state_size, action_size, Player.BLACK, buffer_size=2000, batch_size=32, gamma=0.95, lr=0.00001)
    agent2 = DQNAgent(state_size, action_size, Player.WHITE, buffer_size=2000, batch_size=32, gamma=0.95, lr=0.00001)
    ...
    for epoch in range(num_epochs):
        ...
        for step in range(max_steps):
            ...
            next_state, done, winner = env.step(action, players[0].agent_name)
            history.append((state, action, next_state, done))
            ...
            if done:
                ...
                for s, a, n_s, d in history:
                    agent1.push(s,a,agent1_reward,n_s,d)
                    agent2.push(s,a,agent2_reward,n_s,d)
