...
from ddz.model.model_play_card import PlayCardModel
from ddz.config.config_play_card import ConfigPlayCard
...
class EvaluatorPlayCard(EvaluatorBase):
    def __init__(self, state_shape, action_dim, training=True):
        super(EvaluatorPlayCard, self).__init__()
        self.config = ConfigPlayCard()
    ...
    def create_model(self):
        config = self.config
        model = PlayCardModel(config.filter, self.action_dim, config.resduals)
        model.build((1,) + self.state_shape)
    ...
    def load(self, model_path):
        if not self.restored:
            print("load play model weight:{}".format(model_path))
